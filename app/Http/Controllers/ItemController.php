...
class ItemController extends Controller
{
    ...
    {
		// $this->logToSlack('Next Milestone: Realmsz Deployment on AWS. 🔥', ['request' => $request->all()]);
        try {
    ...
    public function destroy(Request $request)
    {
        try {
            ...
            $this->logToSlack('Exception during item deletion:', ['exception' => $e->getMessage()]);
        }
    }
    ...
    public function archive(Request $request)
    {
        try {
            ...
            $item->archived = $item->archived === 1 ? 0 : 1;
            $item->save();

            return $item->archived === 1 ? 'Item archived.' : 'Item unarchived.';
        } catch (Exception $e) {
            $this->logToSlack('Exception during item\'s archived status change:', ['exception' => $e->getMessage()]);
        }
    }
    ...
    public function priority(Request $request)
    {
        try {
            ...
            $this->logToSlack('Exception during item\'s priority change:', ['exception' => $e->getMessage()]);
        }
    }
    ...
    public function date(Request $request)
    {
        try {
            ...
            $this->logToSlack('Exception during item\'s date change:', ['exception' => $e->getMessage()]);
        }
    }
